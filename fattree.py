...
parser.add_argument('--bandwidth', dest='bandwidth', type=str, default='1M', help='the size of bandwidth')
...
def install_proactive(net, topo):
	...
	for sw in topo.EdgeSwitchList:
		...
		# hao,2018.9.5.tyoe=select,bucket=output:1,bucket=output:2
		# hao,2018.9.6.odd number switches select bucket=output:1; and even number switches select bucket=output:2
		if topo.pod == 4:
			# if int(sw) % 2 != 0:
			...
			# else:
			# 	cmd = "ovs-ofctl add-group %s -O OpenFlow13 \
			# 	'group_id=1,type=select,bucket=output:2'" % sw
...
def traffic_generation(net, topo, flows_peers):
	...
	# Start the servers.
	#hao server is the last of the flows_peers, and the client is the first of the flows_peers. eg [("h001","h006")],h001 is the client and h006 is the server
	serversList = set([peer[1] for peer in flows_peers])     
	for server in serversList:
		# filename = server[1:]
		server = net.get(server)
		# server.cmd("iperf -s > %s/%s &" % (args.output_dir, 'server'+filename+'.txt'))
		server.cmd("iperf -s -u> /dev/null &" )   # Its statistics is useless, just throw away.

	time.sleep(3)

	# Start the clients.
  # linux/unix fork create child process
	# i = 1
	# ret = os.fork()
	# if ret > 0:
	# 	print "---1---"
	# 	for src, dest in flows_peers[0:(len(flows_peers) / 2)]:
	# 		print "----1-%d--"%i
	# 		server = net.get(dest)
	# 		client = net.get(src)
	# 		# filename = src[1:]
	# 		# client.cmd("iperf -c %s -t %d > %s/%s &" % (server.IP(), args.duration, args.output_dir, 'client'+filename+'.txt'))
	# 		client.cmd("iperf -u -c %s -t %d -b %s >> ./iperf_msg.txt" % (server.IP(), 10, str("4M")))   # Its statistics is useless, just throw away. 1990 just means a great number.#hao use 1Mbps rate to tansimate data
	# 		time.sleep(1)
	# 		i += 1
	# else:
	# 	print "---2----"
	# 	for src, dest in flows_peers[(len(flows_peers) / 2):len(flows_peers)]:
	# 		print "----2-%d--"%i
	# 		server = net.get(dest)
	# 		client = net.get(src)
	# 		client.cmd("iperf -u -c %s -t %d -b %s >> ./iperf_msg.txt" % (server.IP(), 10, str("3M")))   # Its statistics is useless, just throw away. 1990 just means a great number.#hao use 5Mbps rate to tansimate data
	# 		time.sleep(1)
			# i += 1
	...
	# 	# filename = src[1:]
	# 	# client.cmd("iperf -c %s -t %d > %s/%s &" % (server.IP(), args.duration, args.output_dir, 'client'+filename+'.txt'))
	# 	# client.cmd("iperf -u -c %s -t %d -b %s > /dev/null &" % (server.IP(), 10, args.bandwidth))   # Its statistics is useless, just throw away. 1990 just means a great number.
	# 	client.cmd("iperf -u -c %s -t %d -b %s >> ./iperf_msg_%d.txt" % (server.IP(), 30, args.bandwidth, i)) 
	# 	time.sleep(1)
	# 	i += 1

  # multiprocessing-> Process class create child processes
	def iperf(server, client,i):
		client.cmd("iperf -u -c %s -t %d -b %s >> ./iperf_msg_%d.txt" % (server.IP(), 10, args.bandwidth, i))
		time.sleep(1)
	...
	for src, dest in flows_peers:
		...
		server = net.get(dest)
		client = net.get(src)
		prs.append(multiprocessing.Process(target=iperf, args=(server, client,i)))
		i+=1
...
def run_experiment(pod, density, ip="127.0.0.1", port=6633, bw_c2a=10, bw_a2e=10, bw_e2h=10):
	...
	fanout = args.k
	Controller_Ryu = Popen("/home/ubuntu/ryu/bin/ryu-manager --observe-links ./EFattree.py --k_paths=%d --weight=bw --fanout=%d" % (4, 4), shell=True, preexec_fn=os.setsid)
