...
def get_average_bisection_bw(value_dict, traffics, app):
	#hao,the length of traffics is 80
	value_list = []
	complete_list = []
...
def plot_results():
	...
	# num_groups = len(traffics_brief)
	# num_bar = len(apps)
	# Reroute_value_list = get_value_list_2(throughput, traffics, item, 'Reroute')
	...
	# index = np.arange(num_groups) + 0.15
	# bar_width = 0.1
	# plt.bar(index + 0 * bar_width, Reroute_value_list, bar_width, color='#315E9B', label='Reroute')
	# plt.bar(index + 1 * bar_width, ECMP_value_list, bar_width, color='#FB8025', label='ECMP')
	# plt.bar(index + 2 * bar_width, Hedera_value_list, bar_width, color='#21A27A', label='Hedera')
	# plt.xticks(index + num_bar / 2.0 * bar_width, traffics_brief, fontsize='large')
	...
	# num_groups = len(traffics_brief)
	# num_bar = len(apps)
	# Reroute_value_list = get_value_list_2(first_packet_delay, traffics, item, 'Reroute')
	...
	# index = np.arange(num_groups) + 0.15
	# bar_width = 0.15
	# plt.bar(index, Reroute_value_list, bar_width, color='r', label='Reroute')
	...
	# num_groups = len(traffics_brief)
	# num_bar = len(apps)
	# Reroute_value_list = get_value_list_3(first_packet_delay, traffics, items, 'Reroute')
	...
	num_groups = len(traffics_brief)
	num_bar = len(apps)
	# Reroute_value_list = get_value_list_3(average_delay, traffics, items, 'Reroute')
